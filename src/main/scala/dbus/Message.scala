...
trait Message {
  ...
  case class HeaderError(error: ErrorName)             extends MessageHeader { val code: Byte = 4; val f: Field = FieldVariant(error.name)}
  case class HeaderReplySerial(serial: Int)            extends MessageHeader { val code: Byte = 5; val f: Field = FieldVariant(FieldWord32(serial))}
  ...
  case class MethodReturn(
    ...
    val headers = List(HeaderReplySerial(replySerial).some, messageSignature(body).toOption map (HeaderSignature(_)), sender map (HeaderSender(_)), destination map (HeaderDestination(_))) filter (_.isDefined) map (_.get)
  }
  ...
  case class Error(
    ...
    val flags = List(NoReplyExpected)
    val headers = List(HeaderError(error).some, HeaderReplySerial(replySerial).some, messageSignature(body).toOption map (HeaderSignature(_)), sender map (HeaderSender(_)), interface map (HeaderInterface(_)), destination map (HeaderDestination(_))) filter (_.isDefined) map (_.get)
  }
